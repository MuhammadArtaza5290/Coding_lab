    ...
    <div className='relative pb-4 '>
        <div>
            ...
            return <div key={item.id} className={parseInt(item.id)===slideIndex ? "opacity-100 duration-700 ease-in-out scale-100" : "opacity-0 duration-700 ease-in-out scale-95"}>
                <div>
                    {(item.id)=== slideIndex && (
                <img className="h-[600px] w-full" src={item.image} alt={item.title} />
                    )}
                </div>
                <div className="absolute top-44 mx-auto inset-x-1/4">
                
                    <p className='text-white text-4xl font-inter font-bold tracking-normal leading-none'>{item.text}</p>
                    {(item.id)=== slideIndex && item.text}
                </div>
            </div>
        })}</div>
        {/* here we made circle at bottom of the image */}
        <div className='flex absolute bottom-12 left-[45%]'>
            {sliderData.map((dot, index)=> {
                return <div className='mr-4'  key={dot.id}>
                    <div className={index === slideIndex ? 'bg-green-300 rounded-full p-2 cursor-pointer' : 'bg-white rounded-full p-2 cursor-pointer'}
                     onClick={() => dispatch(dotSlide(index))}></div>
                </div>
            })}
        </div>
        {/* here we made two arrow at right and left of the image */}
